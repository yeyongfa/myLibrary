...
require_once("config.php"); 
	...
	if($_POST['log_username']){
		...
		$sql="select * from gdutuser where userid='$username'";
		$rs=mysql_query($sql);
	...
	if($_POST['log_psw']){
		...
		$pwd=md5($_POST['log_psw']);
		$sql="select * from gdutuser where userid='$username2' and password='$pwd'";
		$rs=mysql_query($sql);
		if(mysql_num_rows($rs)!=1){
	...
	if($_POST['log_submit']){
		...
		if($code==$_SESSION["auth"]){
			...
			$pwd2=md5($_POST['log_psw2']);
			$sql="select * from gdutuser where userid='$username3' and password='$pwd2'";
			$rs=mysql_query($sql);
			if(mysql_num_rows($rs)==1){
				$_SESSION["pwd"]=$_POST["log_psw2"];
				$_SESSION["userid"]=$_POST["log_username3"];
